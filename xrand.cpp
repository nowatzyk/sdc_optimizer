 ...
 *  rnd_ri (int n)	: returns positive integers [0,n-1]
...
#define LC_C 3907864577u			/* result of a int trial & error series    */

#define Xrnd(x) (x * LC_A + LC_C)   /* the LC polynomial			*/
			
static unsigned Fib[55];	        /* will use X(n) = X(n-55) - X(n-24)	*/
static int Fib_ind;					/* current index in circular buffer		*/
static unsigned Xrnd_var;	        /* LCA - recurrence variable		*/
static unsigned auxtab[256];        /* temporal permutation table		*/
static unsigned prmtab[64] = {      /* spatial permutation table		*/
...
//
// modified: seed 0-31 use precomputed stuff
//
{
    unsigned u;
    int i;
    ...
    assert (sizeof(x) == 2 * sizeof(unsigned));
...
{ 
    ...
    t = &auxtab[(j >> 24) & 0xfflu];
    i = *t;
    Xrnd_var = *t = Xrnd(Xrnd_var);
    t = &prmtab[j & 0x3clu];

    j =  *t++ & i;
    j |= *t++ & ((i << 24) | ((i >>  8) & 0x00fffffflu));
    j |= *t++ & ((i << 16) | ((i >> 16) & 0x0000fffflu));
    j |= *t   & ((i <<  8) | ((i >> 24) & 0x000000fflu));
    
    return j & 0x7ffffffflu;
}
 ...
 *
 * Note: this code assumes that 2 'unsigned's can hold a 'double'
 ...
 */
{
    ...
    //printf("mu0=%08x mu1=%08x tu0=%08x tu1=%08x\n", man.u[0], man.u[1], t.u[0], t.u[1]);
    q = t.d - 1.0;
    //printf(">>>>>>> rnd_01d() = %.6f\n", q);
    return q;
    /*    return t.d - 1.0; */
